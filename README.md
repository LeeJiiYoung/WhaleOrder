...
외부 결제 API(토스페이먼츠) 실패 시 데이터 정합성을 유지합니다.

```
결제 승인 성공 → 주문 생성 → Kafka 큐 등록
결제 승인 실패(토스 거절) → 주문 CANCELLED → 재고 복구 → PaymentFailedException
...
> 아래 결과는 Mock PG(가상 결제, 90% 성공/10% 실패) 시절 `POST /api/payments` 단일 엔드포인트를 대상으로 측정한 기록입니다. 현재는 실제 토스페이먼츠 결제창을 거치는 `prepare`/`confirm` 2단계 흐름이라 브라우저 상호작용 없이는 동일한 방식으로 재현할 수 없어, 과거 측정치를 그대로 보존합니다. 멱등성 처리·분산 락 등 검증된 아키텍처 자체는 결제 수단이 바뀌어도 동일하게 적용됩니다.

k6로 점진적 부하 (1명 → 50명 동시 접속, 2분간) 측정 — 선결제(`POST /api/payments`, Mock PG) 기준
...
> 아래 결과도 Mock PG 시절 `POST /api/payments` 기준 측정치입니다(위 부하 테스트 결과 항목 참고). 분산 락으로 재고 차감을 직렬화하는 로직 자체는 결제 승인 경로(현재는 `confirm`)와 분리되어 있어 동일하게 유효합니다.
...
> 아래 결과도 Mock PG 시절 `POST /api/payments` 기준 측정치입니다. SHA-256 멱등성 키로 동일 요청을 1건만 처리하는 로직은 현재 `prepare`/`confirm` 양쪽에 동일하게 적용되어 있습니다(자세한 내용은 [Payment 도메인 문서](docs/wiki/domains/payment.md) 참고).
